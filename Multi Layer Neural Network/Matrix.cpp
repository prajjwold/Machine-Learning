...
Matrix& Matrix::subset(int rows, int cols){
	if(getNumRows()<=rows && getNumCols()<=cols){
			cerr<<"Matrix \t["<<getNumRows()<<"*"<<getNumCols()<<"]is smaller in dimension to subsize to dimension: ["<<rows<<"*"<<cols<<"]"<<endl;
			throw "Matrix is smaller in dimension to subsize";
	}
	Matrix *b=new Matrix(rows,cols);
	for(int i=0;i<rows;i++){
		for(int j=0;j<cols;j++){
			b->getData()[i][j]=getData()[i][j];
		}
	}
	return *b;
}
...
Matrix& Matrix::random(const int row,const int col,const double minRange, const double maxRange){
	srand((unsigned)time(0));
	Matrix* a=new Matrix(row,col);
	for(int i=0;i<row;i++){
		for(int j=0;j<col;j++){
			a->data[i][j] =minRange +( (maxRange-minRange) * static_cast<double>(rand())/static_cast<double>(RAND_MAX));
...
Matrix& Matrix::random(const int row,const int col){
	...
	for(int i=0;i<row;i++){
		for(int j=0;j<col;j++){
			a->data[i][j] = static_cast<double>(rand()) / static_cast<double>(RAND_MAX);
		}
	}
	return *a;
}
